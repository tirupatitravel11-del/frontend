...
const ETIOS_MODELS = ["Toyota Etios", "Toyota Etios Cross"];

interface EtiosHeroProps {
  from: string;
  to: string;
  startingFare?: number;
}
...
export default function EtiosHero({
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const message = `Hello, I want to book a Toyota Etios taxi.
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:gap-14 sm:px-6 sm:py-16 lg:grid-cols-2 lg:py-24">
        ...
        <div>
          ...
          <span className="mb-4 inline-block rounded-full border border-gold/30 bg-gold/10 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-gold sm:px-4 sm:text-xs">
            Toyota Etios Available
          </span>

          {/* Route Label */}
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-gold sm:text-sm">
            {from} → {to} Toyota Etios Service
          </p>

          {/* H1 */}
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl md:text-5xl lg:leading-tight">
            {from} to {to} <span className="text-gold">Etios Taxi</span> Service
            at Affordable Fares
          </h1>

          {/* Description */}
          <p className="mt-4 max-w-xl text-sm leading-6 text-slate-600 sm:mt-5 sm:text-base sm:leading-7">
            Book a comfortable and reliable Toyota Etios from {from} to {to}.
            Ideal for families, couples, business travel and airport transfers
            with comfortable seating, massive boot space and powerful AC.
          </p>
