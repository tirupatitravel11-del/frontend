...
interface TempoTravellerHeroProps {
  from: string;
  to: string;
  startingFare?: number;
  fare:any
...
export default function TempoTravellerHero({
  from,
  to,
  startingFare = 3500,
  fare
}: TempoTravellerHeroProps) {
  const [tripType, setTripType] = useState<TripType>("one-way");
  const [model, setModel] = useState(TEMPO_MODELS[0]);
    ...
    try {
      const response = await axios.post(
        process.env.apiUrl + "/api/create-booking-cab",
        {
          ...
          from,
          to,
          fare:tripType == "one-way" ? fare?.oneWayPrice : fare?.roundTripPrice,
          vehicle: model,
          trip:tripType
        },
        {
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-12 sm:gap-14 sm:px-6 sm:py-16 lg:grid-cols-2 lg:py-24">
        ...
        <div>
          ...
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-gold sm:text-sm">
            {from} → {to} Tempo Traveller Service
          </p>

          <h1 className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl md:text-5xl lg:leading-tight">
            {from} to {to}{" "}
            <span className="text-gold">Tempo Traveller</span> Service at
            Fixed Fares
          </h1>

          <p className="mt-4 max-w-xl text-sm leading-6 text-slate-600 sm:mt-5 sm:text-base sm:leading-7">
            Travel comfortably with our spacious and air-conditioned Tempo
            Travellers. Perfect for family trips, group travel, corporate
            journeys and outstation tours with experienced drivers.
          </p>

          {/* Tempo Traveller Specs */}
          <div className="mt-6 grid grid-cols-3 gap-2 sm:mt-8 sm:gap-4">
            <div className="rounded-xl border border-slate-200 bg-white p-3 text-center shadow-sm sm:p-4">
              <p className="text-xl font-bold text-gold sm:text-2xl">12–24</p>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-3 text-center shadow-sm sm:p-4">
              <p className="text-xl font-bold text-gold sm:text-2xl">AC</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xl sm:p-8">
          ...
          <form onSubmit={handleBook} className="mt-4 space-y-4 sm:mt-5">
            {/* Tempo Traveller Selection */}
            <div>
              <label className="mb-1.5 block text-sm font-semibold text-slate-700">
                Select Tempo Traveller
              </label>
