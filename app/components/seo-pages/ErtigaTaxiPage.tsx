...
// Ertiga specific components (create these based on your Innova/Urbania components)
// import ErtigaFitGuide from "@/_components/Ertiga/ErtigaFitGuide"; // Adapt from InnovaCrystaFitGuide
...
const ERTIGA_MODELS = [
  ...
  "Maruti Suzuki Ertiga 6 Seater (Captain Seats)",
];
          ...
          <div className="grid w-full items-center gap-12 lg:grid-cols-2">
            ...
            <div className="max-w-3xl">
              <span className="mb-4 inline-block rounded-full border border-gold/30 bg-gold/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-gold">
                Reliable & Fuel-Efficient MPV
              </span>

              <h1 className="text-4xl font-bold leading-[1.1] tracking-tight text-slate-900 sm:text-5xl md:text-6xl">
                {page.title || "Maruti Suzuki Ertiga Rental"}{" "}
                <span className="text-gold">at Fixed Fares</span>
              </h1>

              <p className="mt-6 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
                {page.description ||
                  "Enjoy a smooth, spacious, and budget-friendly travel experience with the Maruti Suzuki Ertiga. Perfect for small families, weekend getaways, and comfortable outstation trips."}
              </p>
              ...
              <div className="mt-8 grid grid-cols-3 gap-3 sm:gap-4">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3 text-center shadow-sm sm:p-4">
                  <Fuel className="mx-auto mb-2 h-6 w-6 text-gold" />
                  <p className="text-lg font-bold text-slate-900 sm:text-xl">
                    Efficient
                  </p>
                  <p className="mt-1 text-[11px] font-medium uppercase tracking-wide text-slate-500 sm:text-xs">
...
function ErtigaBookingForm() {
  const [tripType, setTripType] = useState<TripType>("one-way");
  const [model, setModel] = useState(ERTIGA_MODELS[0]);
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const message = `Hello, I want to book a Maruti Suzuki Ertiga.
...
Please share the fare and availability.`;
  ...
  return (
    <div className="rounded-3xl border border-slate-200/60 bg-white p-6 shadow-2xl shadow-slate-200/50 sm:p-8">
      <div className="mb-6">
        <h3 className="text-2xl font-bold text-slate-900">Book Your Ertiga</h3>
