...
// Innova Crysta specific components
...
const INNOVA_MODELS = [
  ...
  "Toyota Innova Crysta 6 Seater (Captain Seats)",
];

type TripType = "one-way" | "round-trip";

export default function InnovaCrystaPage({ page }: { page: SeoPageData }) {
        ...
        <div className="grid w-full items-center gap-12 lg:grid-cols-2">
          ...
          <div className="max-w-3xl">
            <span className="mb-4 inline-block rounded-full border border-gold/30 bg-gold/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-gold">
              Premium Toyota Innova Crysta Available
            </span>

            <h1 className="text-4xl font-bold leading-[1.1] tracking-tight text-slate-900 sm:text-5xl md:text-6xl">
              {page.title || "Luxury Innova Crysta Rental"}{" "}
              <span className="text-gold">at Fixed Fares</span>
            </h1>

            <p className="mt-6 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg">
              {page.description ||
                "Experience unmatched comfort and reliability with the Toyota Innova Crysta. Perfect for family vacations, corporate travel, and long-distance journeys with premium amenities and professional chauffeurs."}
            </p>
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 sm:gap-4">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-3 text-center shadow-sm sm:p-4">
                <Car className="mx-auto mb-2 h-6 w-6 text-gold" />
                <p className="text-lg font-bold text-slate-900 sm:text-xl">
                  Premium
                </p>
                <p className="mt-1 text-[11px] font-medium uppercase tracking-wide text-slate-500 sm:text-xs">
...
function InnovaBookingForm() {
  ...
  const handleBook = (e: React.FormEvent) => {
    ...
    const message = `Hello, I want to book a Toyota Innova Crysta.
